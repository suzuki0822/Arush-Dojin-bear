...
def get_all_post_titles():
    ...
    page = 1
    while True:
        response = requests.get(post_api_url, auth=(api_user, api_password), params={'per_page': 100, 'page': page, 'status': 'any'})
...
# 除外したいジャンルIDのリスト
excluded_genre_ids = ['558', '152002', '4076', '514', '4127', '153006', '7110', '7111', '153008', '153010', '153011', '153012', '30104', '153016', '8', '12', '160006', '160004', '160103']
filtered_items = []
for item in data['result']['items']:
    # 各商品に対して、ジャンルIDを取得
    item_genre_ids = [genre['id'] for genre in item.get('iteminfo', {}).get('genre', [])]
...
for item in filtered_items:
    ...
    if normalized_title_to_create in existing_titles:
        print(f'記事「{normalized_title_to_create}」は既に存在しています。スキップします。')
        continue
    ...
    # 最初の画像のURLを取得
    response = requests.get(f"{media_api_url}/{first_image_id}", auth=(api_user, api_password))
    ...
    for img_url in image_urls:
        ...
        headers = {"Content-Type": "image/jpeg", "Content-Disposition": "attachment; filename=newimage.jpeg"}
        ...
        if response.status_code == 201:
            ...
            image_info = requests.get(f"{blog_url}/wp-json/wp/v2/media/{image_id}").json()
    ...
    for tag_name in tag_names:
        ...
        if len(tags) > 0:
            ...
            response = requests.post(tag_api_url, json=tag_data, auth=(api_user, api_password))
            if response.status_code == 201:
    ...
    for maker_name in maker_names:
        response = requests.get(group_api_url, params={'search': maker_name}, auth=(api_user, api_password))

        if response.status_code == 200:
            existing_categories = response.json()
            if existing_categories:
                existing_category = existing_categories[0]  # 一致するカテゴリが複数ある場合は最初のものを取得
                ...
                response = requests.post(group_api_url, auth=(api_user, api_password), json=category_data)
    ...
    post_response = requests.post(post_api_url, json=post_data, auth=(api_user, api_password))
    if post_response.status_code == 201:
        print(f'記事「{title}」が正常に投稿されました。')
    else:
        print(f'記事「{title}」の投稿に失敗しました。ステータスコード: {post_response.status_code}')
else:
