<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<div class="entry-content">
			...
			<div class="coment-box">
				<?php foreach ($terms as $term) : ?>
					...
					<div class="chara-text">
						<p>今日はサークル<span class="chara-text-title"><?php echo esc_html($term->name); ?></span>さんの作品、<span class="chara-text-title">『<?php the_title() ?>』</span>を紹介しちゃうよ！買う前に中身をちょっと見たいよね？そんなあなたにうってつけの記事だよ！</p>
					</div>
				<?php endforeach; ?>
			</div>
		...
		<div class="single-content-group-bottom">
			<img src="<?php echo get_template_directory_uri(); ?>/img/chara-icon.png" class="chara-icon" alt="chara-icon">
			<?php foreach ($terms as $term) : ?>
				<p>他のサークル<span class="single-content-group-bottom-title">「<a href="<?php echo get_term_link($term); ?>"><?php echo esc_html($term->name); ?>」</span>さんの作品はこちら！</a></p>
			<?php endforeach; ?>
		</div>
	</div><!-- .entry-content -->
	...
	$current_post_id = get_the_ID(); // 現在の投稿IDを取得
	$current_post_tags = wp_get_post_tags($current_post_id); // 現在の投稿のタグを取得


	$tag_ids = array();
	foreach ($current_post_tags as $tag) {
		$tag_ids[] = $tag->term_id; // タグのIDを配列に追加
	}
	...
	if ($is_cat_455) {
		...
		$related_posts_args = array(
			...
			'orderby' => 'rand', // ランダムに表示する場合
			'tag__in' => $tag_ids, // 現在の投稿と同じタグを持つ投稿を含める
		);
	...
	if (!empty($tag_ids)) {
		$related_posts_args['orderby'] = 'tag__in'; // タグに基づいて並び替え
	}
